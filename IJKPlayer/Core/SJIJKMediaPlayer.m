...
//    NSLog(@"********* %@ *********",[self isVideoToolboxOpen]==YES?@"已开启 VideoToolbox 硬解码":@"未开启 VideoToolbox 硬解码");
...
- (void)seekToTime:(CMTime)time completionHandler:(nullable void (^)(BOOL))completionHandler {
    ...
    if ( ceil(secs) == ceil(self.duration) ) secs = secs;
    if ( isPlaybackEnded ) {
        [self play];
...
- (CMTime)_adjustSeekTimeIfNeeded:(CMTime)time {
    if ( _trialEndPosition != 0 && CMTimeGetSeconds(time) >= _trialEndPosition ) {
        time = CMTimeMakeWithSeconds(_trialEndPosition, NSEC_PER_SEC);
    }
    return time;
}
