...
- (void)_switchControlLayerIfNeeded {
    ...
    if (self.switcher.currentIdentifier == 102) {
        return;
    }
    // 资源出错时
    // - 发生错误时, 切换到加载失败控制层
    ...
    // - 当前如果没有网络, 则切换到无网空制层
    //
    else if ( self.sj_isTimeout || SJReachability.shared.networkStatus == SJNetworkStatus_NotReachable ) {
        if (!_isCache) {
        ...
        if ( self.switcher.currentIdentifier == SJControlLayer_LoadFailed ||
            ...
            [self.defaultEdgeControlLayer.loadingView stop];
            [self.switcher switchControlLayerForIdentifier:SJControlLayer_Edge];
        }
    }
}
